...
class change:
    def __init__(self):
        ...
        self.titlechangea = Label(self.change, text="CHANGE UR PASSWORD", font=("calibri", 20, 'bold'))
        ...
        changebtn=Button(self.change,text="CHANGE", font=("calibri", 20, 'bold'),command=self.changep)
    ...
    def changep(self):
            ...
            if self.new!=self.renter:
                ...
                con = connect()
                cur = con.cursor()
                q = f"select * from user where  email='{self.email}' and password='{self.old}' "
                cur.execute(q)
                result = cur.fetchone()
                print(result)
                ...
                if result is not None:
                    ...
                    q = f"update user set password='{self.new}' where email='{self.email}'"
                    cur.execute(q)
                    ...
                    con.commit()
                    # self.admin.destroy
                    msg.showinfo('', "Password updated Successfully! Now user can Login with new password", parent=self.change)
                else:
